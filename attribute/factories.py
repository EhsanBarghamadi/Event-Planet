...
class EventAttributeValueFactory(factory.django.DjangoModelFactory):
    ...
    value_text = factory.LazyAttribute(
        lambda obj: fake.sentence() if obj.attribute.data_type == Attribute.DataType.TEXT else None
        )
    value_int = factory.LazyAttribute(
        lambda obj: fake.random_int() if obj.attribute.data_type == Attribute.DataType.INTEGER else None
    )
    value_bool = factory.LazyAttribute(
        lambda obj: fake.boolean() if obj.attribute.data_type == Attribute.DataType.BOOLEAN else None
    )
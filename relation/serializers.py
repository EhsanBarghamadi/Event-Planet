...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        check_registration = Registration.objects.filter(
            event=locked_event, participant=request.user
        )
        ...
        if num_participant >= event.capacity:
            raise serializers.ValidationError({"event": "ظرفیت این دوره تکمیل شده است"})
...
class ResultSerializer(serializers.ModelSerializer):
    ...
    participant_id = serializers.PrimaryKeyRelatedField(
        source="participant", queryset=CustomUser.objects.all(), write_only=True
    )
    ...
    def validate(self, attrs):
        ...
        if request.user != event.organizer:
            raise serializers.ValidationError({"user": "شما مالک این رویداد نیستید!"})
        if event.status != "FINISHED":
            raise serializers.ValidationError(

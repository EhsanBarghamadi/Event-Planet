    ...
    def validate_start_date(self, value):
        ...
        if self.instance:
            ...
            raise serializers.ValidationError(
                "تاریخ شروع رویداد نمی‌تواند در گذشته باشد"
            )
        return value
        ...
        if self.instance:
            ...
            if new_status and original_status != new_status:
                if not Event.is_transition_allowed(original_status, new_status):
                    ...
                    allowed_statuses = (
                        [status.value for status in allowed_list]
                        if allowed_list
                        else ["هیچکدام"]
                    )
                    raise serializers.ValidationError(
...
class EventStageSerializer(serializers.ModelSerializer):
    ...
    event_id = serializers.PrimaryKeyRelatedField(
        source="event", queryset=Event.objects.all(), write_only=True, required=False
    )
    ...
    def validate(self, attrs):
        ...
        if not self.instance and not event:
            raise serializers.ValidationError({"event_id": "رویداد الزامی است!"})

        if self.instance:
            if "event" in attrs:
                if event != self.instance.event:
                    raise serializers.ValidationError(
                        {"event": "تغییر رویداد مجاز نیست!"}
                    )
        ...
        overlapping = EventStage.objects.filter(
            event=event, start_time__lt=end_time, end_time__gt=start_time
        )
